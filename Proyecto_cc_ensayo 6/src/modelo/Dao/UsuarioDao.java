     ...
     public String verificarArchivo(String usu, String contraseña) throws IOException {
        ...
        if(!archivo.exists()){
            ...
            System.out.println("El archivo existe");
             ...
             valida2 = leerArchivo2(usu, contraseña); 
            if(valida.equals("Gerente CC")){
            valida = "Gerente CC";
          }else if (valida.equals("Cliente")){
              valida = "Cliente";
          } else if (valida.equals("Administrador Local")){
              valida = "Administrador Local";
          }else if (valida2.equals("Vendedor")){
              valida = "Vendedor";
          }else if (valida2.equals("Inventario")){
              valida = "inventario";
          }
       ...
       public String leerArchivo2(String usu, String contraseña) throws FileNotFoundException, IOException{
       File archivo2 = new File("PersonalLocal.txt");
           String validacion = "otro";
        String linea = null;
        int numeroRegistros=0;
        BufferedReader leerFichero = new BufferedReader(new FileReader(archivo2));
        while ((linea = leerFichero.readLine()) != null) { 
            numeroRegistros+=1;
        }
        leerFichero.close();
        if(numeroRegistros==0)
            ...
            String [][]datos = new String[numeroRegistros][8];
            int posicion=0;
            String line = null;
            String tempUsuario = "";
            String tempClave = "";
            BufferedReader leerArchivo = new BufferedReader(new FileReader("PersonalLocal.txt"));
            while ((line = leerArchivo.readLine()) != null) {   
                StringTokenizer mistokens = new StringTokenizer(line,"\t");
                datos[posicion][0] = mistokens.nextToken().trim(); // nombre
                datos[posicion][1] = mistokens.nextToken().trim(); // apellido
                datos[posicion][2] = mistokens.nextToken().trim();// usuario
                datos[posicion][3] = mistokens.nextToken().trim(); // contraseña
                datos[posicion][4] = mistokens.nextToken().trim(); // correo
                datos[posicion][5] = mistokens.nextToken().trim(); // rol
                datos[posicion][6] = mistokens.nextToken().trim(); // local
                datos[posicion][7] = mistokens.nextToken().trim(); // usuario admin
